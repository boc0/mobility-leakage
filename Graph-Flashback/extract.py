...
def evaluate_difficulty(args, data_file, device, users_original):
    ...
    with torch.no_grad():
        for batch in dataloader_test:
            ...
            for j in range(active_users.size(0)):
                ...
                order = np.argsort(-o_n, axis=1)  # descending order indices per timestep
                seq_len = len(y_j)
                # Compute ranks for each prefix setting
                for p in args.prefix_lengths:
                    p_eff = min(p, seq_len)  # guard
                    ranks = []
                    for k in range(p_eff, seq_len):
                        t_label = int(y_j[k].item())
                        rank = int(np.where(order[k] == t_label)[0][0]) + 1
                        ranks.append(rank)
                    user_ranks_by_prefix[p][u_local].extend(ranks)

    # Aggregate to mean rank per prefix
    results_global = {}
    for u_local in range(poi_loader.user_count()):
        g_uid = local_to_global[u_local]
        results_global[g_uid] = {}
        for p in args.prefix_lengths:
            ranks = user_ranks_by_prefix[p][u_local]
            if ranks:
    ...
    for g in missing_globals:
        results_global[g] = {p: None for p in args.prefix_lengths}

    return results_global


def write_results(output_dir, base_name, results, users_original, prefix_lengths):
    os.makedirs(output_dir, exist_ok=True)
    out_path = os.path.join(output_dir, f"{base_name}_extraction.csv")
    with open(out_path, 'w') as f:
        header = ["tid"] + [f"prefix-{p}" for p in prefix_lengths]
        f.write(",".join(header) + "\n")
        for global_uid in sorted(results.keys()):
...
def main():
    parser = argparse.ArgumentParser(description="Compute extraction difficulty (rank-based) for Graph-Flashback model.")
    ...
    for fpath in files:
        ...
        results = evaluate_difficulty(a, fpath, device, users_original)
        out_csv = write_results(args.output_dir, base, results, users_original, args.prefix_lengths)
