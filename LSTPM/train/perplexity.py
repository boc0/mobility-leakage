...
def main():
    parser = argparse.ArgumentParser(description="Compute per-trajectory perplexity for an LSTPM model on a .pk dataset")
    ...
    parser.add_argument('--output', type=str, default=None, help='Optional CSV output path (user,session,perplexity)')
    parser.add_argument('--distance', type=str, default=None, help='Optional path to distance.pkl; falls back to building from vid_lookup')
    ...
    # Distance: load if provided; else try auto next to pk; else build from lookup
    ...
    if args.distance and os.path.exists(args.distance):
        ...
        auto_path = os.path.join(os.path.dirname(args.data_pk), 'distance.pkl')
    ...
    # Model sizes from checkpoint to avoid mismatch
    ...
    with torch.no_grad():
        for u_idx in sorted(data_neural.keys()):
            ...
            for sid in sorted(sessions.keys()):
                ...
                # print(predictions_logp.shape, padded_seq_t[:, 1:].shape, mask_ix_t[:, :-1].shape)
